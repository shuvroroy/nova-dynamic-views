<template>
  <LoadingView :loading="initialLoading">
    <template v-if="shouldOverrideMeta && resourceInformation && title">
      <Head :title="__(':resource Details: :title', {
    ...
    <div v-if="shouldShowCards && hasDetailOnlyCards">
      <Cards v-if="cards.length > 0" :cards="cards" :only-on-detail="true" :resource="resource" :resource-id="resourceId"
        :resource-name="resourceName" />
    </div>
    ...
    <div :class="{
      ...
      <component :is="resolveComponentName(panel)" v-for="panel in panels" :key="panel.id" :panel="panel"
        :resource="resource" :resource-id="resourceId" :resource-name="resourceName" class="mb-8">
        <div v-if="panel.showToolbar" class="md:flex items-center mb-3">
          <div class="flex flex-auto truncate items-center">
            ...
            <Badge v-if="resource.softDeleted" :label="__('Soft Deleted')"
          ...
          <div class="ml-auto flex items-center">
            ...
            <!-- Actions Menu -->
            <DetailActionDropdown v-if="resource" :resource="resource" :actions="actions" :via-resource="viaResource"
              :via-resource-id="viaResourceId" :via-relationship="viaRelationship" :resource-name="resourceName"
              class="mt-1 md:mt-0 md:ml-2 md:mr-2" @actionExecuted="actionExecuted" @resource-deleted="getResource"
              @resource-restored="getResource" />

            <Link v-if="showViewLink" v-tooltip="{
                placement: 'bottom',
                distance: 10,
                skidding: 0,
                content: __('View'),
              ...
              data-testid="view-resource" dusk="view-resource-button" tabindex="1">
            <BasicButton component="span">
              <Icon type="eye" />
            </BasicButton>
            </Link>

            <Link v-if="resource.authorizedToUpdate" v-tooltip="{
                placement: 'bottom',
                distance: 10,
                skidding: 0,
                content: __('Edit'),
              ...
              class="rounded hover:bg-gray-200 dark:hover:bg-gray-800 focus:outline-none focus:ring"
              data-testid="edit-resource" dusk="edit-resource-button" tabindex="1">
            <BasicButton component="span">
